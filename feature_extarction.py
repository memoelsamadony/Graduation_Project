...
def feature_extraction(Input_ID):  
    ...
    if os.path.exists(dssp) == False:
        return  error_code_dic["software path error dssp"],""

    if os.path.exists(PSIBLAST) == False:
        return  error_code_dic["software path error blast"],""    
    
    if os.path.exists(HHBLITS) == False:
        return  error_code_dic["software path error hmm"],""
    
    if os.path.exists(UR90) == False:
        return  error_code_dic["database path error 90"],""
   
    if os.path.exists(HHDB) == False:
        return  error_code_dic["database path error 30"] ,""  
